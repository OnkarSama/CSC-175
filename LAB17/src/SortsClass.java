...
public class SortsClass {
    ...
    public static void selectionSort(ArrayList<Comparable> theList) {
        int n = theList.size();
        for (int last = n - 1; last >= 1; last--) {
            int largest = indexOfLargest(theList,last+1);
            Comparable temp = theList.get(largest);
            theList.set(largest, theList.get(last));
            theList.set(last, temp);
        }
    }

    public static int indexOfLargest(ArrayList<Comparable> theList, int endIndex) {
        int maxIndex = 0;
        for(int i = 1; i < endIndex; i++) {
            if(theList.get(i).compareTo(theList.get(maxIndex)) > 0) {
                maxIndex = i;
            }
        }
        return maxIndex;
